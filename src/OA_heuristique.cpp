...
double Somme2xProba(vector<double> & vecteur_proba, int executeddata, int AddSizeDeadline) {
    // Parcours de Vector et sommation
    double sommeProba=0;
    for (int i=executeddata; i<(int)(vecteur_proba.size())+AddSizeDeadline; i++) {
        sommeProba+=i*i*vecteur_proba[i-AddSizeDeadline];
    }
    return sommeProba;
}
double standard_deviation(vector<double> SizeVector) {
    double variance = Somme2xProba(SizeVector,0,1) - pow(SommexProba(SizeVector,0,1),2);
        ...
        if (!(etat_courant[0].empty())) {
            for (int i=0; i<etat_courant.size(); i++) {
                ...
                if (deadline!=1) {
                    ...
                    bool Ajout_std_real_Video_Case = false; // @PUT to true if you want the output of real case study
                    if (Ajout_std_real_Video_Case) {
                        double std = standard_deviation(size_proba);
                        double constant = deadline+2;
                        double additionalWork = constant*std;
                        wichap += additionalWork;
                    }
                } else {
